...
def run_priority_preemptive(process_list):
    """
    Run Priority Preemptive scheduling algorithm on a list of Process objects.
    ...
    completed = 0
    n = len(process_list)
    ready_queue = []  # (priority, arrival, process) tuples for heap
    ...
    while completed < n:
        ...
        while i < n and processes[i].arrival == time:
            # Use (priority, arrival, process) tuple for heap
            # Lower priority number = higher priority
            heapq.heappush(ready_queue, (processes[i].priority, processes[i].arrival, processes[i]))
            i += 1
        ...
        _, _, current = heapq.heappop(ready_queue)
        ...
        if current.remaining == 0:
            ...
            # Put current back (may be preempted if higher priority arrived)
            heapq.heappush(ready_queue, (current.priority, current.arrival, current))
